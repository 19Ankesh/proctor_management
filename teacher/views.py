...
def meeting_management(request):
    ...
    meetings = ProctorMeeting.objects.filter(proctor__user=request.user).order_by('-date', 'time')
...
def schedule_meeting(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Create the meeting
            meeting = ProctorMeeting.objects.create(
                proctor=Proctor.objects.get(user=request.user),
                date=meeting_date,
                time=meeting_time
            )
            ...
            for student in students:
                Notification.objects.create(
                    ...
                    message=f'A proctor meeting has been scheduled for {meeting_date.strftime("%A, %B %d")} at {meeting_time.strftime("%I:%M %p")}',
...
def teacher_meetings(request):
    ...
    teacher = request.user.teacher
    meetings = ProctorMeeting.objects.filter(proctor__user=teacher.user).order_by('date', 'time')
...
def teacher_meeting_details(request, meeting_id):
    ...
    meeting = get_object_or_404(ProctorMeeting, id=meeting_id, proctor__user=request.user)
