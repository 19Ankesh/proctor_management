...
def student_timetable(request):
    ...
    time_slots = TimeSlot.objects.filter(students=student)
...
def student_meetings(request):
    ...
    student = request.user.student
    meetings = ProctorMeeting.objects.filter(students=student).order_by('-date', '-time')
    
    # Get recent completed meetings with summaries
    recent_completed_meetings = meetings.filter(
        is_completed=True
    ).select_related('summary')[:5]  # Show last 5 completed meetings
